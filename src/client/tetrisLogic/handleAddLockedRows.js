...
/*   Updated: 2020/01/07 20:03:27 by eduwer           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
const handleAddLockedRows = (state, numberOfRows) => {
  const newBoard = [
    ...
    ...Array.from(Array(Math.min(numberOfRows, 30)), () => Array.from(Array(10), () => -1)),
  ];
  ...
  const maxY = Array.from(Array(state.piece.pos.y), (uselsees, i) => state.piece.pos.y - i)
    .reduce((acc, val) => {
      if (val < acc)
        return acc;
      let pos = {x: state.piece.pos.x, y: val};
      if (canMovePiece(newBoard, state.piece.piece, pos))
        return val;
      return acc;
    }, -1);
    if (maxY === -1) {
